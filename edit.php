...
$erscheinungsjahr = ($book['erscheinungsjahr'] !== null && (string)$book['erscheinungsjahr'] !== '0000')
    ...
    : '';
$gekauftBei = (string)($book['gekauft_bei'] ?? '');
...
{
    $regalValue = trim((string)($currentLocation['regal'] ?? ''));
    $fachValue = trim((string)($currentLocation['regalfach'] ?? ''));
    $schuberValue = trim((string)($currentLocation['schuber'] ?? ''));

    if ($regalValue !== '')
    {
        $regalSelection = $regalValue;
    }

    if ($fachValue !== '')
    {
        $regalfachSelection = $fachValue;
    }

    if ((int)($currentLocation['ist_im_schuber'] ?? 0) === 1 && $schuberValue !== '')
    {
        $schuberChecked = true;
        $schuberSelection = $schuberValue;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST')
{
    ...
    elseif ($action === 'delete')
    {
        ...
        {
            $errors[] = 'The deletion was not confirmed correctly.';
            $showDeleteConfirmation = true;
        }
        else
        {
            ...
            {
                $errors[] = 'The book could not be deleted: ' . $e->getMessage();
                $showDeleteConfirmation = true;
            }
        }
    }
    else
    {
        ...
        if (empty($errors))
        {
            ...
            $newLocation = normalizeLocationPayload($resolvedRegal, $resolvedRegalfach, $schuberChecked, $resolvedSchuber);
            ...
            try
            {
                updateBook(
                    ...
                    $newGenresInput,
                    $newLocation['regal'],
                    $newLocation['regalfach'],
                    $newLocation['ist_im_schuber'],
                    $newLocation['schuber'],
                    date('Y-m-d')
                );
...
<section class="danger-card">
    ...
    <?php else: ?>
        ...
        <div class="alert alert-error">
            ...
            If you continue, this book will be permanently deleted.
        </div>
        ...
        <div class="form-actions">
            ...
            <form method="post" action="">
                ...
                <button class="btn btn-danger" type="submit">
                    Yes, permanently delete this book
                </button>
            </form>
